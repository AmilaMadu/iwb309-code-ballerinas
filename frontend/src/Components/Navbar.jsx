...
const Navbar = () => {
  ...
  const [showDropdown, setShowDropdown] = useState(false); // Dropdown initially false
  const [activeButton, setActiveButton] = useState(''); // State for active button
  const { user_id, setUserId } = useContext(AppContext);

  useEffect(() => {
    // Ensure the dropdown is closed after login or any page navigation
        ...
        {user_id ? (
          <div className='relative'>
            ...
            {showDropdown && ( // Show dropdown only when showDropdown is true
              <div 
                className='absolute top-full right-0 mt-2 bg-white border rounded-md shadow-lg z-50 p-4 w-48' // Set z-index to 50 for visibility
              >
                <p 
