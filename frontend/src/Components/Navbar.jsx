        ...
        <div className='flex items-center gap-4'>
            {
                ...
                <div className='flex items-center gap-2 cursor-pointer group relative'>
                    ...
                    <div className='absolute top-0 right-0 pt-14 text-base font-medium text-gray-600 z-20 hidden group-hover:block'>
                        ...
                        <p onClick={()=>{navigate('my-appointments'); scrollTo(0,0)}} className='hover:text-black cursor-pointer'>My Appointments</p>
                        <p onClick={logout} className='hover:text-black cursor-pointer'>Logout</p>
                        </div>
                    </div>
                </div>
                :<button onClick={()=> {navigate('/login'); scrollTo(0,0)}} className='bg-primary text-white px-8 py-3 rounded-full font-light hidden md:block'>Create a Account</button>
            }
            <img onClick={()=>setShowMenu(true)} className='w-6 md:hidden' src={assets.menu_icon} alt="" /> {/* for mobile version view----- mobile phone users */}
